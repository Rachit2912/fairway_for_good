-- Migration: Strict Chronological Draw Publication, Paid-Through Eligibility, and Table Security

-- 1. DROP LEGACY FUNCTION OVERLOAD
DROP FUNCTION IF EXISTS public.publish_monthly_draw(UUID, JSONB);

-- 2. REVOKE DIRECT TABLE WRITES ON DRAW_FINANCIALS AND DRAW_AWARDS
REVOKE INSERT, UPDATE, DELETE ON public.draw_financials FROM authenticated;
REVOKE INSERT, UPDATE, DELETE ON public.draw_awards FROM authenticated;

-- 3. UPDATE LOCK_MONTHLY_DRAW PROCEDURE
-- Verifies paid-through subscription eligibility (current_period_end >= NOW())
...
BEGIN
  ...
  -- Link funding allocations from ALL active paid-through subscribers (current_period_end >= NOW())
...
-- 4. UPDATE PUBLISH_MONTHLY_DRAW PROCEDURE
-- Enforces chronological publication order and atomic single-rollover consumption
...
DECLARE
  v_draw public.draws;
  v_unpublished_earlier_count INTEGER := 0;
...
BEGIN
  ...
  -- A. Enforce chronological publication ordering
  -- Cannot publish a draw if an earlier draw remains unpublished
  SELECT COUNT(*) INTO v_unpublished_earlier_count
  FROM public.draws
  WHERE (year < v_draw.year OR (year = v_draw.year AND month < v_draw.month))
    AND status != 'published';

  IF v_unpublished_earlier_count > 0 THEN
    RAISE EXCEPTION 'Chronological publication guard: Earlier draws must be published before publishing Draw %/%', v_draw.month, v_draw.year;
  END IF;
  ...
  -- C. Consume incoming 5-match jackpot rollover from immediately preceding published draw
